...
func (p *TelegramMessageProcessor) ProcessUpdate(ctx context.Context, update *tgbot.Update) error {
	...
	accountID := strconv.FormatInt(chatID, 10)
	...
	// 確保使用者存在
	if err := p.ensureUser(ctx, accountID); err != nil {
	...
	// 路由到對應的命令處理器
	return p.routeCommand(ctx, command, arg1, arg2, chatID)
}

// ensureUser 確保使用者存在，不存在則建立
func (p *TelegramMessageProcessor) ensureUser(ctx context.Context, accountID string) error {
	_, err := p.userAccountPort.GetOrCreate(ctx, accountID, valueobject.UserTypeTelegram)
	return err
}
