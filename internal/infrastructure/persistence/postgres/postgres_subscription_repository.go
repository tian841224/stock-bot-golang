...
func (r *subscriptionRepository) GetByUserID(ctx context.Context, userID uint) ([]*entity.Subscription, error) {
	...
	err := r.db.WithContext(ctx).Preload("User").Preload("Feature").Where("user_id = ?", userID).Find(&subscriptions).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
...
func (r *subscriptionRepository) GetByFeatureID(ctx context.Context, featureID uint) ([]*entity.Subscription, error) {
	var subscriptions []*models.Subscription
	err := r.db.WithContext(ctx).Preload("User").Preload("Feature").Where("feature_id = ?", featureID).Find(&subscriptions).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
...
func (r *subscriptionRepository) List(ctx context.Context, offset, limit int) ([]*entity.Subscription, error) {
	var subscriptions []*models.Subscription
	err := r.db.WithContext(ctx).Preload("User").Preload("Feature").Offset(offset).Limit(limit).Find(&subscriptions).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
