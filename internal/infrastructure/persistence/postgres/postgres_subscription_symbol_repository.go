...
func (r *subscriptionSymbolRepository) toEntity(model *models.SubscriptionSymbol) *entity.SubscriptionSymbol {
	...
	if model.Subscription != nil {
		result.Subscription = &entity.Subscription{
...
func (r *subscriptionSymbolRepository) toModel(entity *entity.SubscriptionSymbol) *models.SubscriptionSymbol {
	...
	if entity.Subscription != nil {
		model.Subscription = &models.Subscription{
...
func (r *subscriptionSymbolRepository) GetUserSubscriptionStockList(ctx context.Context, userID uint) ([]*entity.SubscriptionSymbol, error) {
	...
	err := r.db.WithContext(ctx).Where("user_id = ? AND feature_id = ?", userID, valueobject.SubscriptionTypeStockInfo).First(&userSubscription).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
	...
	var subscriptionSymbols []*models.SubscriptionSymbol
	err = r.db.WithContext(ctx).Preload("Subscription").Preload("StockSymbol").Where("subscription_id = ?", userSubscription.ID).Find(&subscriptionSymbols).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
